...
public class UploadService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        String chosenAccountName = intent.getStringExtra(youtubeUploadMainActivity.ACCOUNT_KEY);
    ...
    private void tryUploadAndShowSelectableNotification(final Uri fileUri, final YouTube youtube) throws InterruptedException {
        while (true) {
            ...
            if (videoId != null) {
                ...
                Log.v("videoIdHere", videoId);
                SharedPreferences mPrefs = getSharedPreferences("label", 0);
                String videoTitleString = mPrefs.getString("videoTitle", "default_value_if_variable_not_found");
                String videoDescriptionString = mPrefs.getString("videoDescription", "default_value_if_variable_not_found");
                Log.v("videoTitleString", videoTitleString);
                Log.v("videoDescriptionString", videoDescriptionString);
                SharedPreferences.Editor mEditor = mPrefs.edit();
                mEditor.putString("videoUrl", videoId).commit();
                mEditor.putString("videoUploaded", "true").commit();
    ...
    private String tryUpload(Uri mFileUri, YouTube youtube) {
        ...
        try {
            ...
            videoId = ResumableUpload.upload(youtube, fileInputStream, fileSize, mFileUri, cursor.getString(column_index), getApplicationContext(),videoTitleString,videoDescriptionString);
