...
public class t_ChangePassword extends Activity
    ...
    private boolean isGooglePlayServicesAvailable() {
        ...
        Log.v("t_ChangePassword" , "Success");
...
// Change the spreadsheet's title.
        ...
        protected void onPostExecute(List<String> output) {
            ...
            if (output == null || output.size() == 0) {
                ...
                Log.v("t_ChangePassword" , "damn");
            } else {
                output.add(0, " ");
                mOutputText.setText(TextUtils.join("\n", output));
                Log.v("t_ChangePassword" , "Wofdad21");
                displayAvailability();
                saveData();
            ...
            if (mLastError != null) {
                if (mLastError instanceof GooglePlayServicesAvailabilityIOException) {
                    ...
                    startActivityForResult(
                            ...
                            t_ChangePassword.REQUEST_AUTHORIZATION);
                } else {
                    mOutputText.setText("The following error occurred:\n"
                            + mLastError.getMessage());
                    Log.v("t_ChangePassword" , "Worked2");
                }
            } else {
    ...
    public void SubmitData(View view) {
        ...
        sId = "testing123";
        EditText newPassEditText = (EditText) findViewById(R.id.pNewPassword);
        EditText confirmPasswordEditText = (EditText) findViewById(R.id.pConfirmPassword);
        String stPassword = String.valueOf(newPassEditText.getText());
        String ctPassword = String.valueOf(confirmPasswordEditText.getText());

        if (sFName.length() >= 1) {
            if (sLName.length() >= 1) {
                if (sId.length() >= 6) {
                    if (stPassword.length() >= 8) {
                        if (stPassword.equals(ctPassword)) {
                            if (1==1) {
                                if (sPhone.length() >= 10) {
                            ...
                            Toast.makeText(getApplicationContext(), "Above Passwords don't match",
                                    Toast.LENGTH_SHORT).show();
